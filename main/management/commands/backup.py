...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        elif self.engine in ('postgresql_psycopg2', 'postgresql','django.db.backends.postgresql_psycopg2','django.db.backends.postgresql'):
            print(f'Doing Postgresql backup to database {self.db} into {outfile}')
    ...
    def do_postgresql_backup(self, outfile):
        os.popen(f'pg_dump --dbname=postgres://{self.user}:{self.passwd}@{self.host}:{self.port}/{self.db} > {outfile}')
