...
def changelang(request):
    ...
    l = request.META.get('HTTP_REFERER') if request.META.get(
        'HTTP_REFERER') else reverse('Home')
    if old_lang == 'en':
...
def schedulepdf(request, year):
    ...
    if sys.platform.startswith('win32'):
        ...
        config = pdfkit.configuration(
            wkhtmltopdf=settings.BASE_DIR / os.path.join('wkhtmltopdf', 'bin', 'wkhtmltopdf.exe'))
        pdfkit.from_url(
            f"http://{domain}{reverse('schedule print',args=[year])}", False, configuration=config)

        with open(os.path.join(settings.MEDIA_ROOT, 'pdf', f'schedulepdf-{year}.pdf'), "rb") as pdf_file:
            data = pdf_file.read()
        sync_to_async(os.remove(os.path.join(
            settings.MEDIA_ROOT, 'pdf', f'schedulepdf-{year}.pdf')))
        ...
        img = urllib.request.urlopen(
            f'https://image.thum.io/get/width/1920/crop/900/maxAge/1/noanimate/http://{domain+reverse("schedule img",args=[year, 1])}').read()
...
def video(request, year, day):
    ...
    if day in ('E', 'DI', 'T', 'C', 'P'):
        ...
        try:
            maahome = Year.objects.filter(year=int(year)).values(
    ...
    if day == 'MAA':
        videos = Videos.objects.filter(yearmodel=yearid, day__in=[
                                       'E', 'DI', 'T', 'C', 'P'], test=False).select_related('yearmodel')
        try:
            day = videos[0].day
        except IndexError:
            return redirect(reverse('Videos', args=[int(year), 'S']))
    ...
    livevideo = Videos.objects.filter(
        yearmodel=yearid, live=True, test=False).values('day', 'live').exists()
    if not livevideo:
...
def redirect_view_puja(request):
    ...
    try:
        videodict = Videos.objects.filter(
            yearmodel=yearid, live=True, test=False).select_related('yearmodel').values('day').get()
    except:
        videodict = {'day': 'None'}
        try:
            videodict = Videos.objects.filter(yearmodel=yearid, test=False).select_related(
                'yearmodel').values('day').latest('yearmodel', 'day')
...
def qrcode(request, logo=2):
    ...
    return HttpResponse(
        QrGen('https://'+domain+reverse('Redirect'),
              True if logo == 1 else False).gen_qr_code(),
...
def durgapujayear(): return relativedelta(
    datetime.datetime.now(), datetime.datetime(2001, 1, 1)).years
...
def generate_thumbnail(request_obj):
    ...
    if request.LANGUAGE_CODE == 'bn' and text.replace('/', '').isdigit():
        font = os.path.join(settings.BASE_DIR, 'main',
                            'static', 'fonts', 'Baloo_Da-Regular.ttf')
        text = bangla.convert_english_digit_to_bangla_digit(str(text))
        d.text(((100-w)/2, (25-h)/2), str(text), fill=str(textc),
               font=ImageFont.truetype(font, 11, layout_engine=ImageFont.LAYOUT_RAQM))
    else:
    ...
    mask = Image.open(os.path.join(settings.BASE_DIR, 'main',
                      'imagesreq', 'mask.png')).convert('L')
